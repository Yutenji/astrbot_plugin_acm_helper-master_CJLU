...
class AcmHelperPlugin(Star):
    ...
    async def connect_db(self):
        ...
        await self.db.execute("CREATE TABLE IF NOT EXISTS users (qq_id TEXT PRIMARY KEY, name TEXT NOT NULL, cf_handle TEXT, luogu_id TEXT, school_oj_uid TEXT, status TEXT, school TEXT, last_sync_timestamp INTEGER DEFAULT 0);")
        # 兼容旧数据库：若没有 school_oj_uid 列则补充
        async with self.db.execute("PRAGMA table_info(users)") as cursor:
            user_columns = [row[1] for row in await cursor.fetchall()]
        if "school_oj_uid" not in user_columns:
            await self.db.execute("ALTER TABLE users ADD COLUMN school_oj_uid TEXT;")
    ...
    async def _generate_rank_image(self, title: str, users_data: list, include_school_oj: bool = False) -> bytes | str:
        ...
        width = 920 if include_school_oj else 800
        ...
        if include_school_oj:
            columns = {'排名': 50, '用户名': 170, 'CF题数': 320, '洛谷题数': 450, '校OJ题数': 580, '总题数': 710, '身份': 840}
        else:
        ...
        for i, user in enumerate(users_data):
            ...
            if include_school_oj:
                draw.text((columns['校OJ题数'], y_text), str(user['school_oj_count']), font=font_body, fill=color_red, anchor='mm')
    ...
    async def _query_rank_data(self, days: int = None, status: str = None, limit: int = None, include_zero: bool = False) -> list:
        params = []
        sql = "SELECT u.qq_id AS qq_id, u.name AS user_name, u.status AS user_status, u.school_oj_uid AS school_oj_uid, COUNT(s.id) AS total_count, COUNT(CASE WHEN s.platform = 'codeforces' THEN 1 END) AS cf_count, COUNT(CASE WHEN s.platform = 'luogu' THEN 1 END) AS luogu_count FROM users u LEFT JOIN submissions s ON u.qq_id = s.user_qq_id"
        where_clauses = []
        ...
        sql += " GROUP BY u.qq_id"
        if not include_zero:
            sql += " HAVING total_count > 0"
        sql += " ORDER BY total_count DESC, user_name ASC"
        if limit is not None: sql += f" LIMIT {limit}"
        
        async with self.db.execute(sql, tuple(params)) as cursor: return await cursor.fetchall()

    async def _fetch_school_oj_count(self, session: aiohttp.ClientSession, uid: str) -> int:
        if not uid:
            return 0
        url = f"https://acm.cjlu.edu.cn/api/users/{uid}"
        try:
            async with session.get(url, timeout=10) as response:
                response.raise_for_status()
                data = await response.json(content_type=None)
            if isinstance(data, dict) and data.get("code") == 200:
                solved = data.get("data", {}).get("solved", [])
                if isinstance(solved, list):
                    return len(solved)
            logger.warning(f"[校OJ] 查询 UID={uid} 返回异常结构: {json.dumps(data, ensure_ascii=False)[:200]}")
            return 0
        except Exception as e:
            logger.warning(f"[校OJ] 查询 UID={uid} 失败: {e}")
            return 0

    async def _merge_school_oj_counts(self, users_data: list) -> list:
        if not users_data:
            return []

        async with aiohttp.ClientSession() as session:
            tasks = [self._fetch_school_oj_count(session, (row['school_oj_uid'] or '').strip()) for row in users_data]
            school_counts = await asyncio.gather(*tasks, return_exceptions=True)

        merged = []
        for row, cnt in zip(users_data, school_counts):
            safe_cnt = cnt if isinstance(cnt, int) and cnt > 0 else 0
            item = dict(row)
            item['school_oj_count'] = safe_cnt
            item['total_count'] = int(item.get('total_count', 0)) + safe_cnt
            merged.append(item)

        merged = [x for x in merged if int(x.get('total_count', 0)) > 0]
        merged.sort(key=lambda x: (-int(x.get('total_count', 0)), x.get('user_name', '')))
        return merged
    ...
    async def cmd_total_rank(self, event: AstrMessageEvent):
        title = "生涯总过题排行榜"
        users_data = await self._query_rank_data(days=None, limit=None, include_zero=True)
        users_data = await self._merge_school_oj_counts(users_data)
        if not users_data: yield event.plain_result("📊 生涯总榜暂无数据。"); return
        image_bytes = await self._generate_rank_image(title, users_data, include_school_oj=True)
        if isinstance(image_bytes, str): yield event.plain_result(image_bytes)
        else: yield event.chain_result([Image.fromBytes(image_bytes)])
